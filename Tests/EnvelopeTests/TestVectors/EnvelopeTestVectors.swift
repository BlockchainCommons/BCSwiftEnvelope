...
final class EnvelopeTestVectors: XCTestCase {
    ...
    @StringBuilder
    private func formatDocument(testCases: [TestCase]) -> String {
        documentHeader("Envelope Test Vectors")

        header2("Overview")

        paragraph("This document provides test vectors for `envelope`. It is generated by `EnvelopeTestVectors.testGenerateEnvelopeTestVectors()` in the `BCSwiftEnvelope` test suite.")

        header2("Status")

        paragraph("Draft, with a reference implementation in this package.")
    ...
    struct TestCase {
        ...
        func format() -> String {
            ...
            monospaced(envelope.diagnostic(annotate: true, context: formatContext))

            header3("Tagged CBOR Annotated Binary")
            monospaced(envelope.dump(annotate: true, context: formatContext))
