...
public extension Envelope {
    ...
    func sign<T>(with privateKeys: PrivateKeyBase, note: String? = nil, using rng: inout T) -> Envelope
    ...
    ///
    /// - Returns: The signed envelope.
    func sign(with privateKeys: PrivateKeyBase, note: String? = nil) -> Envelope {
    ...
    ///
    /// - Returns: The signed envelope.
    func sign<T>(with privateKeys: [PrivateKeyBase], using rng: inout T) -> Envelope
    where T: RandomNumberGenerator
    {
        privateKeys.reduce(into: self) {
            $0 = $0.sign(with: $1, using: &rng)
        }
    }
    ...
    ///
    /// - Returns: The signed envelope.
    func sign(with privateKeys: [PrivateKeyBase]) -> Envelope {
        var rng = SecureRandomNumberGenerator()
    ...
    ///
    /// - Returns: The signed envelope.
    func sign<T>(with privateKeys: PrivateKeyBase, uncoveredAssertions: [Envelope], using rng: inout T) throws -> Envelope
    where T: RandomNumberGenerator
    {
    ...
    ///
    /// - Returns: The signed envelope.
    func sign(with privateKeys: PrivateKeyBase, uncoveredAssertions: [Envelope]) throws -> Envelope {
        var rng = SecureRandomNumberGenerator()
        return sign(with: privateKeys, using: &rng)
    }
}
